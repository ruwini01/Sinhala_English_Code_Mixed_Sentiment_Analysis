...
- **Schema (raw, 11 columns):** `id, text, sentiment_label, source_platform, source_url, language_type, clean_text, text_length, domain, content_type, emotion`. `clean_text` is regenerated by the pipeline from `text` regardless of what the raw file supplies (documented 7-step rule, see `ml/DATA.md`); it is the model input column, never `text`.
- **Note:** the dataset is **kept local** (not committed) while it is being finalized and professionally annotated. The preprocessing scripts (`ml/src/preprocess/`) regenerate the `ml/data/processed/` splits from it on demand. Full provenance (sha256, row counts, per-step rationale) lives in `ml/DATA.md`.
...
Everything lives under `ml/` (the frontend demo is a separate `frontend/` Next.js app).
...
│   ├── data/
│   │   ├── raw/               # immutable source CSV (local, git-ignored except README.md)
│   │   ├── interim/            # pipeline intermediates (regenerated, git-ignored)
│   │   ├── processed/          # splits/ (committed, LOCKED) + tokenized/ (git-ignored)
│   │   ├── lexicon/            # Sinhala sentiment / romanized-Sinhala lexicons
│   │   ├── embeddings/         # fastText vectors for B2
│   │   └── annotations/        # NLPC-UOM gold LID annotations
│   ├── src/
│   │   ├── common/             # schema.py (single source of truth for columns/paths/labels), seed, logger
│   │   ├── ingest/              # YouTube / Facebook / e-commerce collectors
│   │   ├── preprocess/          # validate_schema -> quarantine -> clean_text -> language_id -> make_splits -> tokenize_cache
│   │   ├── train/                # run_tfidf_lr, run_bilstm_fasttext, hf_trainer, run_mbert_full, run_xlmr_full, run_lora_scl_lid
│   │   ├── models/               # model architectures (LoRA+SCL+LID proposed model, LID embedding layer)
│   │   ├── eval/                  # metrics, statistical tests, make_figures.py
│   │   ├── explain/               # SHAP, LIME, Sinhala lexical analysis, faithfulness test
│   │   └── serve/                  # FastAPI inference endpoint
│   ├── notebooks/              # EDA + Colab training/explainability notebooks
│   ├── results/                # figures/, tables/, reports/, run-output JSONs
│   ├── DATA.md                 # provenance ledger (sha256, row counts, per-step rationale)
│   └── requirements.txt
├── frontend/                  # Next.js demo UI, calls ml/src/serve/api.py
└── README.md
```
...
cd Sinhala_English_Code_Mixed_Sentiment_Analysis/ml
...
> Preprocessing, EDA, and the TF-IDF baseline (B1) run on CPU locally. The neural models (B2–B4 and the proposed model) need a GPU — run those on **Google Colab**. All commands below are run from `ml/`.

---

## Usage

**1. Preprocess the raw dataset → train/val/test splits** (each step reads/writes paths defined in `src/common/schema.py`):
```bash
...
python -m src.preprocess.make_splits   # refuses to run if splits already exist
...
Neural baselines, the proposed model, ablations, and explainability are run from the Colab notebooks in `notebooks/` (GPU required).
